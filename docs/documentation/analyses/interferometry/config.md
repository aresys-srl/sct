...
```toml title="Interferometric Analysis section"
...
??? danger "Validation"
    ...
    ``azimuth_blocks_number`` and ``range_blocks_number`` are actually automatically computed if not explicitly set from
    configuration.
