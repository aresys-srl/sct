...
def TemporaryDirectoriesAndConfFile() -> Generator[tuple[Path, Path, Path, Path], Any, None]:
    ...
    with (
        ...
        TemporaryDirectory() as second_product,
        TemporaryDirectory() as output_dir,
        TemporaryDirectory() as conf_dir,
    ):
        conf_file = Path(conf_dir).joinpath("conf.toml")
        yield Path(input_product), Path(second_product), Path(output_dir), Path(conf_file)


class InterferometricAnalysisCLITestCase(unittest.TestCase):
    """Test interferometric analysis command line interface"""

    def setUp(self) -> None:
        self.cli_runner = CliRunner()
        self.command = "interferometric-analysis"
        self.test_configuration = SCTInterferometricAnalysisConfig()
    ...
    def test_invalid_product(self):
        ...
        with TemporaryDirectoriesAndConfFile() as (product, second_product, output_dir, conf_file):
    ...
    def test_invalid_product_1(self):
        ...
        with TemporaryDirectoriesAndConfFile() as (product, second_product, output_dir, conf_file):
            self.test_configuration.to_toml(conf_file)
            command_args = (
                f"--config {conf_file} {self.command} -p {product} -pp {second_product} -out {output_dir}".split()
            )
            result = self.cli_runner.invoke(
                sct_analysis,
                command_args,
